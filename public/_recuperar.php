...
if (isset($args[0]) && !empty($args[0]) && !empty($dataset_json)) {
    ...
    foreach($metadata as $columna) {
        ...
        if (strpos($columna['column_name'], '_borrado') !== false) {
             $campo_borrado = $columna['column_name'];
        }
    }

    if (!empty($campo_borrado)) {
        ...
        foreach ($dataset as $data) {
            ...
            if (isset($data['id']) && !empty($data['id'])) {
                ...
                $sql = "UPDATE {$tabla} SET {$prefijo}borrado=null WHERE {$prefijo}id = {$data[id]}";
            }
        ...
        $respuesta_dataset = array(array('ERROR' => 'El registro no permite ser recuperado.'));
    }

...
if (!empty($dataset_json)) {
    ...
    if (isset($dataset->cedula) && !empty($dataset->cedula)) {
        ...
        if (isset($dataset->reiniciar) && !empty($dataset->reiniciar)) {
            ...
            $password = md5($cedula);
            $result = q("UPDATE sai_usuario SET usu_password='$password' WHERE usu_id=$id RETURNING *");
        } else if (isset($dataset->borrar) && !empty($dataset->borrar)) {
            if ($_SESSION['cedula'] != $cedula) {
                $result = q("UPDATE sai_usuario SET usu_borrado=now() WHERE usu_id=$id RETURNING *");
            } else {
            ...
            if ($count_usuarios_cedula == 0) {
                ...
                $sql = ("UPDATE sai_usuario SET $sql_update WHERE usu_id=$id RETURNING *");
                $result = q($sql);
            } else {

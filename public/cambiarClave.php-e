...
if (isset($_POST['oldpassword']) && !empty($_POST['oldpassword']) && isset($_POST['new1password']) && !empty($_POST['new1password']) && isset($_POST['new2password']) && !empty($_POST['new2password'])){
    ...
    $password = q("SELECT usu_password FROM sai_usuario WHERE usu_id=$usu_id")[0]['usu_password'];

    if (md5($old) == $password) {
        if ($new1 == $new2) {

            $result = q("UPDATE sai_usuario SET usu_password='".md5($new1)."' WHERE usu_id=$usu_id");

...
$campos = q("SELECT * FROM sai_campo_extra");
?>
<h2>Campos</h2>
...
<tr class="<?php echo (empty($campo['cae_borrado']) ? ($campo['cae_password'] == md5($campo['cae_cedula']) ? 'alert alert-info' : '') : 'alert alert-danger'); ?>">
    <th><?php echo ($i+1).'.&nbsp;'; ?></th>
    <td><span id=""><a href="#" onclick="p_abrir('<?=$campo['cae_id']?>');return false;"><?=$campo['cae_cedula']?></a></span></td>
    <td><span id="nombre_<?=$campo['cae_id']?>"><?php echo $campo['cae_apellidos'].' '.$campo['cae_nombres']; ?></span></td>
    <td><span id="rol_<?=$campo['cae_id']?>"><?=$campo['rol']?></span></td>
    <td><span id="correo_electronico_<?=$campo['cae_id']?>"><?=$campo['cae_correo_electronico']?></span></td>
  </tr>
<?php endforeach; ?>
</tbody>
</table>
...
<script>
/*
function p_abrir(cae_id){
    $.ajax({
        'url':'/_listar/usuario/'+cae_id
    }).done(function(data){
        data = eval(data);
        usuario = data[0];
        console.log(usuario);
